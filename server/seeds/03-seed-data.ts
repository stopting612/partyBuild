...
export async function seed(knex: Knex): Promise<void> {
    ...
    // //     (acc, cur) => acc.set(cur.price, cur.id),
    ...
    await trx('party_room_bookings')
        .insert(
            partyOpenTime.map((partyRoomOpenTime) => {
                return {
                    party_room_id: PartyRoomMapping.get(partyRoomOpenTime.party_room_id),
                    booking_start_time: partyRoomOpenTime.booking_start_time,
                    booking_end_time: partyRoomOpenTime.booking_end_time,
                    party_id: partyMapping.get(partyRoomOpenTime.party_id)
                };
            })
        );
    ...
    await trx('creator_contact')
        .insert(
            creatorContact.map((creatorOpenTime) => {
                return {
                    user_id: UsersMapping.get(creatorOpenTime.user_id),
                    party_id: partyMapping.get(creatorOpenTime.party_id),
                    phone_number: creatorOpenTime.phone_number,
                    contact_name: creatorOpenTime.contact_name,
                };
            })
        );
    await trx('calculator_option')
        .insert(
            calculatorOption.map((calculatorOption) => {
                return {
                    name: calculatorOption.name,
                    price: calculatorOption.price,
                    number_of_people: calculatorOption.number_of_people,
                    party_id: partyMapping.get(calculatorOption.party_id),
                    status: calculatorOption.status
                };
            })
        );


    await trx('alcohols_suppliers')
        .insert(
    ...
    await trx('alcohol_orders')
        .insert(
            alcoholType.map((alcoholOrders) => {
                return {
                    party_id: partyMapping.get(alcoholOrders.party_name),
                    alcohol_id: alcoholNewMapping.get(alcoholOrders.alcohol_name),
                    price:alcoholOrders.price,
                    special_requirement: alcoholOrders.special_requirement,
                    quantity: alcoholOrders.quantity,
                    states: alcoholOrders.states
                };
            })
        );
    ...
    .insert(
        emailVerificationType.map((emailV) =>{
            return {
                email:emailV.email,
                verification_code:emailV.verification_code,
                username:emailV.username,
                password:emailV.password
