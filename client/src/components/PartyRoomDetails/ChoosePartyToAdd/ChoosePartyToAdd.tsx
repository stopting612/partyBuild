import { IPartyRoomCart } from 'components/PartyRoomDetails/AddToCartForm/AddToCartForm'
import { useEffect, useState } from 'react'
import { useDispatch, useSelector } from 'react-redux'
import { addToParty } from 'redux/partyRoomDetails/action'
...
export default function ChoosePartyToAdd(props: {
  ...
  const partyRoomID = useSelector(
    (state: RootState) => state.partyRoomDetails.id
  )
  ...
  const onClick = (data: IPartyRoomCart) => {
    dispatch(
      addToParty(
        partyRoomID!,
        data.partyID!,
        Number(data.partyRoomPerson),
        data.date!,
        data.startTime!,
        data.endTime!
      )
