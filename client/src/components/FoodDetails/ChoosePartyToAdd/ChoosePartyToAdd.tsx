...
import { addToParty } from 'redux/foodDetails/action'
...
export default function ChoosePartyToAdd(props: {
  ...
  const foodID = useSelector((state: RootState) => state.foodDetails.id)
  ...
  const onClick = (data: ICart) => {
    dispatch(addToParty(foodID!, data.partyID!, data.area, data.quantity))
  }
  return (
    <>
      {!toggle && (
